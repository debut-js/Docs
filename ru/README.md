...
## Примеры и результаты работы стратегии
...
## Enterprise edition
**([Купить подписку](https://www.patreon.com/bePatron?u=57560983) 15$/месяц)**
...
<h5>Синтетическая эмуляция тиков</h5>
Дополнительное разделение тиков большого размера (например если за 1 минуту было большое изменение цены) на составные части двигающиеся хаотично по направлению к изменению цены, таким образом, эмулируя реальное быстрое движение рынка в нужную сторону, обеспечивая размер тика не более 0.75%

<h5>Alpaca </b> поддерживает `5min`,` 15min` и другие временные фреймы debut</h5>
Благодаря аггрегации свечей, Alpaca поддерживает таймфреймы искуственно, не смотря на отсутсвие поддержки на сервере.

<h5> Анализатор отчетов `finder`.</h5>
Создает скриншоты из json файлов отчетов и групирует их по эффективности
...
__Описание:__ Массив открытых позиций [ExecutedOrder](#executedorder) в порядке открытия `this.orders[0]` - первая. Сделки добавляются синхронно в массив, сначала создается оптимистичная сделка, обозначачющая процесс обработки, как только приходит ответ от сервера, сделка заменяется на оригинальную.
...
interface DebutOptions {
    ...
    margin?: boolean; // Переключение счета на маржинальный в binance
    instrumentType?: 'SPOT' | 'FUTURES'; // Тип торгуемого инструмента (пока только для binance), по умолчанию SPOT
...
interface ExecutedOrder {
    // Уникальный идентификатор генерируемый на клиенте
    cid: number;
...
interface Instrument {
    ...
    lotPrecision: number;
     // Тип инструмента (CFD, FUTURES, SPOT...)
    type: InstrumentType;
    // Внутренний идентификатор инструмента Debut
    id: string;
}
```
